...
class NodalDiscontinuousGalerkin():
    ...
    def initial_conditions(self):
        ...
        for k in range(self.K):
            ...
            for j in range(N):
                ...
                self.xij[j + m] = -np.sin(np.pi * self.xi[j + m])
    ...
    # Calculations
    def DGTimeDerivative(self, t, k, xij, ind):
        ...
        xijtd = self.DGDerivative(un, up, xij, k, qn, qp, ind)
        return xijtd

    def DGDerivative(self, un, up, xij, k, qn, qp, ind):
        ...
        q = np.matmul(Dhij, xij) # * np.sqrt(a)
        for j in range(0, len(xij)):
            q[j] = - q[j] - ((up * self.lj1[j] - un * self.ljn1[j])/LGw[j])
            q[j] = q[j] * self.Ji[k] * np.sqrt(a)

        udot = np.matmul(Dhij, q) * np.sqrt(a)
        ux =  np.matmul(Dhij, xij) * xij
        for j in range(0, len(xij)):
    ...
    def plot(self, t, T="N/A", errors=False):
        ...
        for num, val in enumerate(self.xk):
            ...
            y = np.arange(-height + height/8, height, (2 * height / points))
            ...
            if num == len(mesh) - 1:
                ...
                plt.plot(x, y, linestyle='dashed', color='k')
                plt.title(
                    "Discontinuous Galerkin Method - Burger's Equation (N={}, K={}) for T={})".format(
                        self.N, self.K, np.round(T,2)))
        plt.scatter(self.xi, self.xij, s=12, marker='x', label="t={}".format(np.round(t,2)))
        # if(t > t/10):
        #     plt.scatter(self.xi, self.q, s=12, label="t={}".format(np.round(t,2)))
...
def DGStepByRK3(tn, dt, dg):
    ...
    q = np.zeros(dg.size, dtype='float')
    ...
    for m in range(0, 3):
        ...
        for k in range(dg.K):
            ...
            xijdt[ind:ind+N], q[ind:ind+N] = dg.DGTimeDerivative(t, k, dg.xij[ind:ind+N], ind) #dg.J[k]
            ind += N
...
def LegendreCollocationIntegrator(Nt, T, dg, split=False, tol=1.0, tol2=1.0, pL2_lim=0, hL2_lim=0, Kmax = 40, Pmax=14):
    ...
    dg.Nmax = Pmax
    Ntflr = np.floor(T/dt)
    Ntqtr = np.floor(Ntflr/4)
    for n in tqdm(np.arange(0, T, dt)):
        # print(n)
        dg = DGStepByRK3(n, dt, dg)
        # print((n/dt)/T % Ntqtr)
        if np.round((n/dt)/T % Ntqtr) == 0:
            dg.plot(n, T)
...
if __name__ == "__main__":
    ...
    mesh.extend(mesh_obj.mesh_gen(2, -0.5, -0.2))
    mesh.extend(mesh_obj.mesh_gen(2, -0.2, 0.0))
    mesh.extend(mesh_obj.mesh_gen(2, 0.0, 0.2))
    mesh.extend(mesh_obj.mesh_gen(2, 0.2, 0.5))
    mesh.extend(mesh_obj.mesh_gen(1, 0.5, 1.0))

    # INITIALISING SOLVER
    N = 6 # Started tests on 12
    K = len(mesh)
    ...
    for i in np.arange(1.0,1.1 , 0.2):
        DG = NodalDiscontinuousGalerkin(N, K, mesh)
        DG.plot(0.0, i)
        xijout, X = LegendreCollocationIntegrator(((N*K)**2), T+i, DG, split=False, tol=tol, tol2 = tol2, pL2_lim=pL2_lim, hL2_lim=hL2_lim, Kmax=25, Pmax=14)
        print(DG.N_dict)
    # plt.legend()
    # plt.show()
